...
Page({  
  ...
  Recruitments: function() {  
    wx.navigateTo({  
      url: '/pages/profile/profile?type=recruitments'  
    });  
  },  
  
  Comments: function() {  
    wx.navigateTo({  
      url: '/pages/profile/profile?type=comments'  
    });  
  },  
    ...
    wx.showModal({  
      ...
      success: (res) => {  
        if (res.confirm) {  
          ...
          wx.clearStorageSync(); // 清除本地存储，模拟退出登录  
          app.globalData.openid = null; // 清除全局变量中的 openid  
          wx.reLaunch({  
            url: '/pages/dengru/newpage' // 重新跳转到登录页面  
          });  
        }  
      }  
    });  
